...
int main()
{
    ...
    std::array<uint8_t, 4> ip_address_data = {192, 168, 115, 2};

    // gateway address of the virtual TAP interface, which is created by calling init_default_netif()
    std::array<uint8_t, 4> default_gateway_data = {192, 168, 115, 1};

    // netmask of the virtual TAP interface, which is created by calling init_default_netif()
    std::array<uint8_t, 4> netmask_data = {255, 255, 255, 0};
...

    {
        ...
        // create the virtual TAP interface on the unix host
